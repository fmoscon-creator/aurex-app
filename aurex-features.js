...
function _getActivoScores(sym) {
  ...
  // fallback universal: score sintÃ©tico para cualquier ticker no registrado
  var seed=_iaSeed(sym);
  return {tendencia:seed>0.5?0.05:-0.05,rsi:seed>0.6?-0.03:0.03,volumen:0.01,volatilidad:-0.02,correlacion:0.02,oro_petroleo:0,macro:-0.01,earnings:0,macd:0,soporte_resist:0};
}
...
function _updateTotals(items){
  ...
  // Si no hay precios reales, mostrar "Cargando..." â NO mostrar valor falso
  if(!hasPrices){
  ...
  if(el('port-best')) el('port-best').textContent = items.length > 0 ? (bestSym + ' ' + _fmt(bestPct,'pct')) : 'â';
  if(el('port-best-badge')) { el('port-best-badge').textContent = items.length > 0 ? (bestSym + ' ' + _fmt(bestPct,'pct')) : 'â'; el('port-best-badge').style.color = bestPct >= 0 ? '#22c55e' : '#ef4444'; }
  // P&L: mostrar "..." hasta que haya precios reales, luego calcular segÃºn perÃ­odo
...
function _renderThermoRisk(items){
  ...
  items.forEach(function(item){
    ...
    // Sin fallback falso â si no hay senal del backend, queda como SIN SENAL
  ...
  var segs = [];
  if(pAlc>0) segs.push({p:pAlc, c:'#3FB950', l:'ð Alcista',       syms:buckets.ALCISTA.syms});
  if(pBaj>0) segs.push({p:pBaj, c:'#F85149', l:'ð Bajista',       syms:buckets.BAJISTA.syms});
  if(pHC>0)  segs.push({p:pHC,  c:'#D4A017', l:'â¡ Sin direcciÃ³n', syms:buckets.HC.syms});
  if(pSin>0) segs.push({p:pSin, c:'#8B949E', l:'â« Sin seÃ±al',      syms:buckets.SIN.syms});
  ...
  var msg = '';
  if(pBaj >= 50){
    msg = 'ð´ RevisÃ¡ urgente â <span style="color:#F85149;"><b>'+bajSyms+'</b></span> en baja confirmada.<br><span style="color:#8B949E;font-size:9px;">La IA confirmÃ³ caÃ­da. EvaluÃ¡ reducir posiciÃ³n antes de que baje mÃ¡s.</span>';
  } else if(pBaj >= 20){
    msg = 'â ï¸ Vigilar: <span style="color:#F85149;"><b>'+bajSyms+'</b></span> con seÃ±al bajista.<br><span style="color:#8B949E;font-size:9px;">SeÃ±al dÃ©bil de baja. Monitorea de cerca antes de decidir.</span>';
  } else if(pAlc >= 50){
    msg = 'ð¢ Buen momento â <span style="color:#3FB950;"><b>'+alcSyms+'</b></span> con momentum positivo.<br><span style="color:#8B949E;font-size:9px;">La IA confirmÃ³ suba. Buen momento para mantener o aumentar posiciÃ³n.</span>';
  } else if(pHC >= 40){
    msg = 'â¡ EsperÃ¡ seÃ±al antes de operar: <span style="color:#D4A017;"><b>'+hcSyms+'</b></span>.<br><span style="color:#8B949E;font-size:9px;">La IA estÃ¡ monitoreando. Confirma direcciÃ³n cuando el mercado define. VolvÃ© maÃ±ana.</span>';
  } else if(pSin >= 70){
    msg = 'â« Sin datos suficientes hoy â no operar hasta nueva seÃ±al.<br><span style="color:#8B949E;font-size:9px;">La IA necesita mÃ¡s datos. Sin acciÃ³n recomendada por ahora.</span>';
  } else {
    var dom = segs[0];
    msg = '<b>'+dom.p.toFixed(0)+'% '+dom.l+'</b> â cartera con seÃ±ales mixtas.<br><span style="color:#8B949E;font-size:9px;">RevisÃ¡ cada activo individualmente antes de operar.</span>';
  }
  el.innerHTML =
    '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:5px;">' +
      ...
      '</div>' +
      '<div style="font-size:8px;color:#8B949E;letter-spacing:.5px;font-weight:500;">CAPITAL POR SEÃAL IA</div>' +
...
window.showThermoInfo = function(){
  ...
  body.innerHTML = '<div style="color:#E6EDF3;font-size:15px;font-weight:700;margin-bottom:12px;">ð¡ï¸ TermÃ³metro de Riesgo</div>' +
    ...
    '<div style="display:flex;align-items:center;gap:8px;"><div style="width:12px;height:12px;border-radius:50%;background:#D4A017;flex-shrink:0;"></div><div style="font-size:12px;color:#E6EDF3;"><b style="color:#D4A017;">ALTA CONV-IA</b> â La seÃ±al mï¿½s valiosa y rara. MÃ¡xima atenciÃ³n: movimiento fuerte inminente. Solo 1-2 activos por dÃ­a reciben esta seÃ±al.</div></div>' +
...
window.showThermoHelp = function(){
  ...
  box.style.cssText = 'background:#161B22;border:1px solid #30363D;border-radius:14px;padding:20px 18px;max-width:300px;width:88%;box-shadow:0 8px 32px rgba(0,0,0,.8);';
  box.setAttribute('onclick','event.stopPropagation()');
  box.innerHTML =
    '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px;">' +
      '<div style="font-size:13px;font-weight:700;color:#E6EDF3;">ð¡ï¸ CÃ³mo leer el TermÃ³metro</div>' +
      '<div onclick="window._closeThermoHelp()" style="font-size:24px;color:#8B949E;cursor:pointer;line-height:1;padding:4px 6px;-webkit-tap-highlight-color:rgba(0,0,0,0);">&#215;</div>' +
    '</div>' +
    '<div style="display:flex;flex-direction:column;gap:10px;font-size:11px;line-height:1.5;">' +
      '<div><span style="color:#3FB950;font-weight:700;">ð¢ Verde â Alcista</span><br><span style="color:#8B949E;">SeÃ±al confirmada de suba. Buen momento para mantener o aumentar posiciÃ³n.</span></div>' +
      '<div><span style="color:#F85149;font-weight:700;">ð´ Rojo â Bajista</span><br><span style="color:#8B949E;">SeÃ±al confirmada de caÃ­da. EvaluÃ¡ reducir antes de que baje mÃ¡s.</span></div>' +
      '<div><span style="color:#D4A017;font-weight:700;">â¡ Dorado â Sin direcciÃ³n</span><br><span style="color:#8B949E;">Movimiento fuerte inminente sin confirmar. EsperÃ¡ la seÃ±al â no operar todavÃ­a.</span></div>' +
      '<div><span style="color:#8B949E;font-weight:700;">â« Gris â Sin seÃ±al</span><br><span style="color:#8B949E;">La IA no tiene datos suficientes hoy. Sin acciÃ³n recomendada.</span></div>' +
    '</div>' +
    '<div style="margin-top:14px;font-size:10px;color:#8B949E;border-top:1px solid #30363D;padding-top:10px;">El % indica cuÃ¡nto de tu capital estÃ¡ en cada zona. Se actualiza con precios actuales.</div>' +
    '<div onclick="window._closeThermoHelp()" style="margin-top:12px;background:#D4A017;color:#000;border-radius:8px;padding:10px;text-align:center;font-size:12px;font-weight:700;cursor:pointer;-webkit-tap-highlight-color:rgba(0,0,0,0);touch-action:manipulation;">Entendido</div>';
...
// WATCHLIST â Spec TAB_WATCHLIST_SPEC.md completo
// Multiples listas, colores, lista principal, detalle activo
// âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ

var WL_COLORS = ['#D4A017','#3B82F6','#EC4899','#8B5CF6','#EF4444','#10B981','#F59E0B','#6366F1'];
var _wlSelectedList = null;
var _wlNewColor = WL_COLORS[0];
var _wlNewPrimary = false;

// Storage helpers â Supabase (synced with native app)
...
window.wlTogglePrimary = function(){
  ...
  var star = document.getElementById('wl-primary-star');
  if(star) star.textContent = _wlNewPrimary ? 'â­' : 'â';
};
...
window.wlSearchAssets = function(){
  ...
  res.innerHTML = filtered.map(function(a, idx){
    ...
    return '<div onclick="wlAddAsset(\''+a.s+'\',\''+a.n.replace(/'/g,"\\'")+'\',\''+a.tipo+'\')" style="display:flex;align-items:center;gap:8px;padding:8px;border-radius:8px;background:#0D1117;border:0.5px solid #21262D;margin-bottom:4px;cursor:pointer">'+
      logoHtml+
      '<div style="flex:1"><div style="font-size:12px;font-weight:700;color:#E6EDF3">'+a.s+'</div><div style="font-size:10px;color:#555">'+a.n+'</div></div>'+
      ...
      '<span style="font-size:9px;color:#555;background:#21262D;padding:1px 5px;border-radius:4px">'+a.tipo+'</span>'+
    '</div>';
  }).join('');
};
...
// âââ TOGGLE ALERTA WHATSAPP âââ
...
// âââ WATCHLIST â Vista principal con multiples listas (spec 2.1-2.5) ââââ
window.renderWatchCnt = function(){
  ...
  lists.forEach(function(list){
    ...
    html += '<div style="display:flex;align-items:center;gap:4px">'+(list.is_primary?'<span style="font-size:11px">â­</span>':'')+'<span style="font-size:12px;font-weight:700;color:'+list.color+'">'+list.name+'</span></div>';
    html += '<div style="font-size:9px;color:#555;margin-top:3px">'+itemCount+' activos</div>';
    html += '</div>';
  });
  html += '</div>';
  if(lists.length > 2) html += '<div style="position:absolute;right:14px;top:0;bottom:0;display:flex;align-items:center;pointer-events:none"><div style="background:#D4A01730;border-radius:12px;width:24px;height:24px;display:flex;align-items:center;justify-content:center"><span style="font-size:16px;color:#D4A017;font-weight:800">âº</span></div></div>';
  html += '</div>';
  ...
  if(currentList){
    ...
    if(currentList.is_primary) {
      html += '<span style="font-size:8px;font-weight:700;color:#D4A017;background:#D4A01720;padding:2px 6px;border-radius:4px">â­ PRINCIPAL</span>';
    } else {
      html += '<a href="javascript:void(0)" data-wl="setPrimary" data-wl-param="'+currentList.id+'" style="font-size:10px;font-weight:600;color:#8B949E;background:#21262D;padding:4px 8px;border-radius:6px;cursor:pointer;border:1px solid #30363D;text-decoration:none">â Marcar principal</a>';
    }
    html += '<a href="javascript:void(0)" data-wl="shareList" style="font-size:16px;cursor:pointer;padding:4px;text-decoration:none">ð¤</a>';
    var _cmpLabel = 'âï¸ Comparar';
    ...
    if(window._wlCompareMode && window._wlCompareItems && window._wlCompareItems.length >= 2){
      _cmpLabel = 'âï¸ Comparar ' + window._wlCompareItems.length;
    ...
    html += '<div onclick="wlDeleteList(\''+currentList.id+'\')" style="font-size:14px;color:#55555560;cursor:pointer;padding:4px">ð</div>';
    html += '</div>';
  }

  // BotÃ³n comparar (arriba de los activos, siempre visible)
  if(window._wlCompareMode){
    var _cc = window._wlCompareItems ? window._wlCompareItems.length : 0;
    html += '<div style="padding:6px 14px;text-align:center"><span style="font-size:10px;color:#8B949E">'+(_cc >= 2 ? 'â '+_cc+' seleccionados â toca âï¸ Comparar arriba' : 'Selecciona 2 a 5 activos')+'</span></div>';
  }

  // Lista de activos
  if(currentItems.length === 0){
    html += '<div style="text-align:center;padding:40px 20px"><div style="font-size:28px;margin-bottom:8px">ð</div><div style="font-size:13px;color:#8B949E">Lista vacia</div><a href="javascript:void(0)" ontouchstart="wlOpenAddModal()" data-wl="addAsset" style="display:inline-block;margin-top:12px;padding:8px 16px;border-radius:8px;background:#D4A01720;border:1px solid #D4A017;color:#D4A017;font-size:11px;font-weight:600;cursor:pointer">Agregar primer activo</span></div>';
  } else {
    currentItems.forEach(function(item){
      ...
      if(sig && sig.scores){
        ...
        varKeys.forEach(function(k){
          ...
          if(v>0.01) dotsHtml+='<div style="width:6px;height:6px;border-radius:3px;background:#3FB950"></div>';
          else if(v<-0.01) dotsHtml+='<div style="width:6px;height:6px;border-radius:3px;background:#F85149"></div>';
        });
        dotsHtml += '</div>';
      }
      ...
      if(isCompareMode) {
        html += '<div data-wl-compare="'+item.s+'" style="width:22px;height:22px;border-radius:11px;border:2px solid '+(isSelected?'#D4A017':'#21262D')+';background:'+(isSelected?'#D4A017':'transparent')+';display:flex;align-items:center;justify-content:center;margin-right:4px;flex-shrink:0">'+(isSelected?'<span style="color:#000;font-size:12px;font-weight:800">â</span>':'')+'</div>';
      } else {
        // Flechas reordenar
        html += '<div style="display:flex;flex-direction:column;gap:1px;margin-right:2px">';
        html += '<a href="javascript:void(0)" onclick="event.stopPropagation();wlMoveItem('+itemIdx+',-1)" style="font-size:11px;color:'+(itemIdx===0?'#333':'#8B949E')+';width:18px;text-align:center;cursor:pointer;text-decoration:none">â²</a>';
        html += '<a href="javascript:void(0)" onclick="event.stopPropagation();wlMoveItem('+itemIdx+',1)" style="font-size:11px;color:'+(itemIdx===currentItems.length-1?'#333':'#8B949E')+';width:18px;text-align:center;cursor:pointer;text-decoration:none">â¼</a>';
        html += '</div>';
      }
      html += logoHtml;
      ...
      html += '<div style="font-size:11px;color:#8B949E;margin-top:1px">'+item.n+'</div>';
      html += '<div style="display:flex;align-items:center;gap:4px;margin-top:3px"><span style="font-size:10px;font-weight:700;color:'+dirColor+'">'+(dir==='alcista'?'ð':dir==='bajista'?'ð':dir==='alta_conf'?'â¡':'')+ ' '+dirLabel+' '+(prob?prob+'%':'')+'</span></div>';
      html += '</div>';
      html += dotsHtml;
      html += '<div style="text-align:right;flex-shrink:0;margin-left:4px"><span style="font-size:14px;font-weight:700;color:#E6EDF3">'+precioFmt+'</span></div>';
      html += '<div style="display:flex;flex-direction:column;align-items:center;gap:4px;margin-left:4px">';
      html += '<div onclick="wlToggleAlert(\''+item.s+'\',event)" style="font-size:13px;opacity:'+(item.alert_active?'1':'0.3')+';cursor:pointer">ð</div>';
      html += '<div onclick="wlRemoveAsset(\''+item.s+'\',event)" style="font-size:12px;color:#555;cursor:pointer">ðï¸</div>';
      html += '</div></div>';
      ...
      if(isCrypto) html += '<span style="font-size:8px;color:#3FB950;font-weight:700;border:0.5px solid #3FB950;border-radius:3px;padding:0.5px 3px;margin-right:2px">24/7</span>';
      if(mktClosed) html += '<span style="font-size:9px;color:#D4A017;font-weight:700;margin-right:4px">Ult. cierre</span>';
      ...
      ['24h','7d','1m','3m','1y'].forEach(function(p,idx){
        html += '<span style="font-size:9px;padding:2px 4px;border-radius:3px;background:'+(idx===0?'#D4A017':'#21262D')+';color:'+(idx===0?'#0D1117':'#8B949E')+'">'+p+'</span>';
      });
      html += '</div></div>';
      html += '</div>';
    });
...
// âââ DETALLE DE ACTIVO (spec 2.3) â modal identico a IA expandido âââââ
window.wlOpenDetail = function(sym){
  ...
  var html = '';
  html += '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px"><div style="display:flex;align-items:center;gap:12px">'+logoHtml+'<div><div style="font-size:18px;font-weight:700;color:#E6EDF3">'+sym+'</div><div style="font-size:11px;color:#8B949E">'+(act?act.n:sym)+' Â· '+(act?act.tipo:'')+'</div></div></div><div onclick="wlCloseDetail()" style="width:36px;height:36px;border-radius:50%;background:#21262D;display:flex;align-items:center;justify-content:center;font-size:18px;color:#E6EDF3;cursor:pointer">â</div></div>';
  html += '<div style="display:flex;gap:10px;margin-bottom:14px"><div style="flex:1;background:#21262D;border-radius:10px;padding:10px;text-align:center"><div style="font-size:9px;color:#8B949E;margin-bottom:4px">Precio</div><div style="font-size:16px;font-weight:700;color:#E6EDF3">'+_fmt(precio)+'</div></div><div style="flex:1;background:#21262D;border-radius:10px;padding:10px;text-align:center"><div style="font-size:9px;color:#8B949E;margin-bottom:4px">24h</div><div style="font-size:16px;font-weight:700;color:'+(cambio>=0?'#3FB950':'#F85149')+'">'+_fmt(cambio,'pct')+'</div></div></div>';
  ...
  if(sig && sig.motivos && sig.motivos.length>0){html += '<div style="font-size:11px;font-weight:600;color:#8B949E;letter-spacing:0.5px;margin-bottom:6px">JUSTIFICACION DEL ANALISIS</div>';(sig.motivos||[]).slice(0,5).forEach(function(m){html += '<div style="display:flex;gap:6px;margin-bottom:5px"><span style="color:'+dirColor+';font-weight:700">-></span><span style="font-size:11px;color:#C9D1D9;line-height:1.4">'+m+'</span></div>';});}
  html += '</div>';
  if(sig){var objC=dir==='bajista'?'#F85149':'#3FB950';var stC=dir==='bajista'?'#FF9500':'#F85149';var up=sig.upside||0;html += '<div style="display:flex;gap:8px;margin-bottom:12px"><div style="flex:1;background:#21262D;border-radius:8px;padding:8px;text-align:center"><div style="font-size:9px;color:#8B949E;margin-bottom:2px">Objetivo</div><div style="font-size:12px;font-weight:700;color:'+objC+'">'+_fmt(sig.objetivo)+'</div></div><div style="flex:1;background:#21262D;border-radius:8px;padding:8px;text-align:center"><div style="font-size:9px;color:#8B949E;margin-bottom:2px">Stop</div><div style="font-size:12px;font-weight:700;color:'+stC+'">'+_fmt(sig.stop)+'</div></div><div style="flex:1;background:#21262D;border-radius:8px;padding:8px;text-align:center"><div style="font-size:9px;color:#8B949E;margin-bottom:2px">'+(up<0?'Downside':'Upside')+'</div><div style="font-size:12px;font-weight:700;color:'+(up<0?'#F85149':'#3FB950')+'">'+(up>=0?'+':'')+up.toFixed(1)+'%</div></div></div>';}
  if(sig && sig.scores){var sc=sig.scores;var vd=[{k:'tendencia',l:'Tendencia 24h'},{k:'rsi',l:'RSI14'},{k:'volumen',l:'Volumen'},{k:'volatilidad',l:'Volatilidad'},{k:'correlacion',l:'Correlacion BTC/SPY'},{k:'oro_petroleo',l:'Oro/Petroleo'},{k:'macro',l:'Macro FED'},{k:'earnings',l:'Earnings'},{k:'macd',l:'MACD (12/26)'},{k:'soporte_resist',l:'Soporte/Resist. 30d'}];var pV=vd.filter(function(d){return(sc[d.k]||0)>0.01;});var nV=vd.filter(function(d){return(sc[d.k]||0)<-0.01;});var neV=vd.filter(function(d){return Math.abs(sc[d.k]||0)<=0.01;});html+='<div style="margin-bottom:12px"><div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:7px"><span style="font-size:10px;color:#8B949E;font-weight:600">VARIABLES DEL MODELO</span><span style="font-size:10px"><span style="color:#3FB950;font-weight:700">â '+pV.length+' alc</span> Â· <span style="color:#F85149;font-weight:700">â '+nV.length+' baj</span></span></div>';pV.forEach(function(d){html+='<div style="padding:4px 8px;background:#3FB95010;border-left:2px solid #3FB950;border-radius:0 6px 6px 0;margin-bottom:3px"><span style="font-size:10px;color:#3FB950;font-weight:600">â '+d.l+'</span></div>';});nV.forEach(function(d){html+='<div style="padding:4px 8px;background:#F8514910;border-left:2px solid #F85149;border-radius:0 6px 6px 0;margin-bottom:3px"><span style="font-size:10px;color:#F85149;font-weight:600">â '+d.l+'</span></div>';});if(neV.length>0){html+='<div style="display:flex;flex-wrap:wrap;gap:3px;margin-top:2px">';neV.forEach(function(d){html+='<span style="font-size:9px;color:#555;background:#21262D;border-radius:4px;padding:2px 6px">â '+d.l+'</span>';});html+='</div>';}html+='</div>';}
  if(sig){html+='<div style="font-size:10px;color:#8B949E;margin-bottom:6px;font-weight:600">OTROS ESCENARIOS</div><div style="display:flex;gap:6px;margin-bottom:12px">';if(dir!=='alcista')html+='<div style="flex:1;background:#3FB95015;border:1px solid #3FB95040;border-radius:8px;padding:6px;text-align:center"><div style="font-size:9px;color:#3FB950">ALCISTA</div><div style="font-size:13px;font-weight:700;color:#3FB950">'+(sig.prob_alcista||'--')+'%</div></div>';if(dir!=='bajista')html+='<div style="flex:1;background:#F8514915;border:1px solid #F8514940;border-radius:8px;padding:6px;text-align:center"><div style="font-size:9px;color:#F85149">BAJISTA</div><div style="font-size:13px;font-weight:700;color:#F85149">'+(sig.prob_bajista||'--')+'%</div></div>';if(dir!=='alta_conf')html+='<div style="flex:1;background:#D4A01715;border:1px solid #D4A01740;border-radius:8px;padding:6px;text-align:center"><div style="font-size:9px;color:#D4A017">ALTA CONV-IA</div><div style="font-size:13px;font-weight:700;color:#D4A017">'+(sig.prob_alta_conf||'--')+'%</div></div>';html+='</div>';}
  html += '<div style="display:flex;align-items:center;justify-content:space-between;padding:12px;background:#0D2818;border-radius:10px;border:1px solid #25D36630;margin-bottom:12px"><div><div style="font-size:12px;font-weight:600;color:#25D366">ð Alerta WhatsApp</div><div style="font-size:9px;color:#8B949E;margin-top:2px">Recibir cambios de senal para '+sym+'</div></div><label class="toggle-sw"><input type="checkbox"><span class="toggle-slider"></span></label></div>';
  html += '<div onclick="event.stopPropagation();if(typeof _compartirSenal===\'function\')_compartirSenal(\''+sym+'\')" style="width:100%;background:#21262D;border:1px solid #30363D;border-radius:8px;padding:10px;text-align:center;color:#E6EDF3;font-size:12px;font-weight:600;cursor:pointer">ð¤ Compartir senal</div>';
  body.innerHTML = html;
...
// âââ COMPARADOR âââ
window._wlCompareMode = false;
window._wlCompareItems = [];

window.wlStartCompare = function(){
  // Si ya estamos en modo comparar y hay 2+ seleccionados â abrir popup
  if(window._wlCompareMode && window._wlCompareItems && window._wlCompareItems.length >= 2){
    window.wlShowCompare();
    return;
  }
  // Si estamos en modo comparar sin suficientes â salir del modo
  if(window._wlCompareMode){
    window._wlCompareMode = false;
    window._wlCompareItems = [];
    renderWatchCnt();
...
// Cargar datos histÃ³ricos para comparador
...
window.wlShowCompare = function(){
  ...
  var dirIcon = function(d){return d==='ALCISTA'?'ð':d==='BAJISTA'?'ð':'â¡';};
  ...
  var html = '<div style="padding:14px;border-bottom:1px solid #21262D;display:flex;align-items:center;justify-content:space-between"><span style="font-size:15px;font-weight:700;color:#D4A017">âï¸ Comparador AUREX</span><a href="javascript:void(0)" onclick="document.getElementById(\'wl-compare-overlay\').remove();window._wlCompareMode=false;window._wlCompareItems=[];renderWatchCnt();" style="width:32px;height:32px;border-radius:16px;background:#21262D;display:flex;align-items:center;justify-content:center;font-size:16px;color:#E6EDF3;text-decoration:none">â</a></div>';

  // Period buttons â actualizan el contenido in-place sin cerrar el overlay
  ...
  items.forEach(function(t){
    ...
    if(isBest) html += '<div style="background:#D4A01730;border:1px solid #D4A017;border-radius:6px;padding:2px 6px;margin-top:4px;display:inline-block"><span style="font-size:7px;font-weight:800;color:#D4A017">â­ MEJOR PERFORMANCE</span></div>';
    html += '</div>';
  });
  html += '</div>';
  ...
  // Data rows
  var rows = [
    {label:'SeÃ±al IA', fn:function(t){var d=getDir(t);return '<span style="color:'+dirColor(d)+';font-weight:700">'+d+'</span>';}},
  ...
  varDefs.forEach(function(v){
    ...
    items.forEach(function(t){
      ...
      var icon=sc>0.01?'â':sc<-0.01?'â':'â';
  ...
  items.forEach(function(t){
    ...
    html += '<div style="width:120px;text-align:center;font-size:11px"><span style="color:#3FB950;font-weight:700">â'+alc+'</span> Â· <span style="color:#F85149;font-weight:700">â'+baj+'</span></div>';
  });
  html += '</div>';

  // Share button
  html += '<a href="javascript:void(0)" onclick="var m=\'âï¸ AUREX Comparador\\nââââââââââââââââ\\n\';';
  items.forEach(function(t,i){
    html += 'm+=\''+(t===bestSym?'â­ ':'')+dirIcon(getDir(t))+' '+t+' â '+getDir(t)+' '+getProb(t)+'%\\n\';';
  });
  html += 'm+=\'ââââââââââââââââ\\nAUREX IA | aurex.live\';if(navigator.share){navigator.share({title:\'AUREX Comparador\',text:m});}else if(navigator.clipboard){navigator.clipboard.writeText(m);alert(\'Copiado\');}" style="display:block;background:#21262D;border-radius:10px;padding:12px;text-align:center;color:#E6EDF3;font-size:12px;font-weight:600;text-decoration:none;margin-top:16px">ð¤ Compartir comparacion</a>';
...
// âââ COMPARTIR LISTA COMPLETA âââ
...
// Render al cargar â sync from Supabase first
...
window._IA_ACTIVOS = [
  ...
  {s:'AAPL', n:'Apple', tipo:'accion', logo:'https://financialmodelingprep.com/image-stock/AAPL.png', icon:'A', color:'#555', ySymbol:'AAPL'},
  ...
  {s:'GDX', n:'Gold Miners ETF', tipo:'etf', logo:'https://financialmodelingprep.com/image-stock/GDX.png', icon:'G', color:'#D4A017', ySymbol:'GDX'},
  {s:'GDXJ', n:'Jr Gold Miners', tipo:'etf', logo:'https://financialmodelingprep.com/image-stock/GDXJ.png', icon:'G', color:'#D4A017', ySymbol:'GDXJ'},
  ...
  {s:'IAU', n:'iShares Gold', tipo:'etf', logo:'https://financialmodelingprep.com/image-stock/IAU.png', icon:'I', color:'#D4A017', ySymbol:'IAU'},
...
function generarSenalesIA() {
  // FUENTE UNICA: backend Railway â mismos datos que la app nativa iOS
  ...
  fetch('https://aurex-app-production.up.railway.app/api/ia-signals', { cache: 'no-store' })
    ...
    .then(function(data){
      ...
      if (data.signals && data.signals.length > 0) {
        // Usar senales del backend â IDENTICAS a las que ve la app nativa
        ...
        console.log('[AUREX IA] OK â mostrando', sigs.length, 'senales del backend');
        return;
      }
...
window.showIAVariablesPopup = function() {
  ...
  var varDefs = [
    ...
    {k:'oro_petroleo',   n:'6. Oro / Petr&#xF3;leo',         d:'Precios de activos refugio. Oro alto = aversiÃ³n al riesgo. Impacta segÃºn tipo de activo.',p:'Media'},
...
async function _fetchPulseForCategory(cat) {
  ...
  // FALLBACK: calcular localmente (misma lÃ³gica)
...
function _renderFearGreed(containerId) {
  ...
  // Fetch Crypto F&G Alternative.me - cache 60 min (se actualiza 1x dÃ­a)
...
window.editFuturesBanner = function(){
  ...
  var rows = allItems.map(function(item){
    ...
    return '<div style="display:flex;align-items:center;justify-content:space-between;padding:8px 0;border-bottom:1px solid #21262D;">' +
      '<span style="color:#E6EDF3;font-size:13px;">' + lbl + '</span>' +
  ...
  popup.innerHTML =
    '<div style="background:#161B22;border:1px solid #30363D;border-radius:14px;padding:20px;width:88%;max-width:340px;max-height:85vh;overflow-y:auto;">' +
      '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;">' +
        '<span style="font-size:15px;font-weight:700;color:#E6EDF3;">Futuros en banner</span>' +
        '<button onclick="document.getElementById(&apos;aurex-fut-edit-popup&apos;).remove()" style="background:#21262D;border:1px solid #30363D;border-radius:6px;color:#8B949E;font-size:16px;cursor:pointer;width:28px;height:28px;display:flex;align-items:center;justify-content:center;">&#x2715;</button>' +
      '</div>' +
      rows +
      '<button onclick="document.getElementById(&apos;aurex-fut-edit-popup&apos;).remove();if(typeof _renderFuturesBanner===&apos;function&apos;){_renderFuturesBanner(&apos;port-futures-banner&apos;);_renderFuturesBanner(&apos;mkt-futures-banner&apos;);var _tmp=document.createElement(&apos;div&apos;);_tmp.id=&apos;tmp-fut-listo&apos;;_tmp.style.display=&apos;none&apos;;document.body.appendChild(_tmp);_renderFuturesBanner(&apos;tmp-fut-listo&apos;);var _sb=document.getElementById(&apos;combo-slide-b&apos;);if(_sb)_sb.innerHTML=_tmp.innerHTML;document.body.removeChild(_tmp);}" style="width:100%;background:#3FB950;border:none;border-radius:8px;padding:10px;color:#0D1117;font-size:14px;font-weight:700;cursor:pointer;margin-top:14px;">Listo</button>' +
    '</div>';
  document.body.appendChild(popup);
};
...
window.toggleFutPref = function(rawS){
  ...
  if(togEl){
    ...
    togEl.style.background = on ? '#3FB950' : '#333';
...
window._selectPortCurr = function(cur) {
  ...
  var badge = document.getElementById('port-curr-badge');
  if(badge) { var dispCur = (cur === 'USD') ? '$' : cur; badge.textContent = dispCur + ' â¾'; badge.style.color='#000'; badge.style.background='#D4A017'; badge.style.borderColor='#D4A017'; }
  var dd = document.getElementById('port-curr-dropdown');
  if(dd) {
