...
function generarSenalesIA() {
  ...
  fetch('https://aurex-app-production.up.railway.app/api/ia-signals', { cache: 'no-store' })
    ...
    .then(function(data){
      ...
      console.log('[AUREX IA] Backend vacio â reintentando en 3s...');
      setTimeout(function(){ generarSenalesIA(); }, 3000);
    })
    .catch(function(err){
      console.error('[AUREX IA] Error backend:', err, 'â reintentando en 3s...');
      setTimeout(function(){ generarSenalesIA(); }, 3000);
    });
}
...
function _renderIALista(signals, keepLoadingBar) {
  ...
  listEl.innerHTML = filtered.map(function(s, i) {
    ...
    return '<div class="ia-row" id="ia-row-'+i+'" style="border-bottom:1px solid #21262D;">' +
  ...
  try {
    ...
    if (backendData && backendData.scores) {
      ...
      if (catData && catData.value != null) {
        var _pColor=catData.value<=20?"#C62828":catData.value<=40?"#FF6B6B":catData.value<=60?"#D4A017":catData.value<=80?"#3FB950":"#00E676"; var _pEmoji=catData.value<=20?"ð±":catData.value<=40?"ð°":catData.value<=60?"ð":catData.value<=80?"ð":"ð¤"; window._pulseCache[catKey]=Object.assign({},catData,{color:_pColor,emoji:_pEmoji});
