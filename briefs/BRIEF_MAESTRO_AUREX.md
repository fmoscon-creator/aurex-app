...
> **📣⏭️ PRÓXIMA SESIÓN (12-jun) — RETOMAR EL PLAN DE MARKETING COBREX desde su ÚLTIMA VERSIÓN para avanzar.** Plan vive en `marketing/PLAN_MKT_COBREX_v4.md` (v4.6; raw: `https://raw.githubusercontent.com/fmoscon-creator/aurex-app/main/marketing/PLAN_MKT_COBREX_v4.md`). Arrancar leyendo la última versión + el estado del sistema MKT ([[project_sistema_mkt_cobrex]]) y seguir desde ahí. Contexto: con iOS 1.3 + Android 1.0.39 ya publicados en las 2 tiendas, el frente MKT (atrasado) pasa a ser prioridad. Pendiente crítico del plan: adquisición de audiencia (0 seguidores). NADA se publica sin OK formal de Fernando. **⚡ AVANCE 11-jun: la AUTOMATIZACIÓN de canales ya está hecha (4/5 conectados y validados, ver update abajo) → lo que sigue de la Fase 2 es el CONTENIDO por canal al banco de imágenes.**
> **🔴🔗 PENDIENTE DE SEGUIMIENTO (11-jun) — LINKEDIN automatización:** la página `linkedin.com/company/cobrexio` (admin = LinkedIn personal de Fernando) NO se puede postear por API hasta que LinkedIn apruebe la **Community Management API** (scope `w_organization_social`). **Acción:** crear app en `linkedin.com/developers` (logueado con el LinkedIn personal de Fernando) + asociarla a la página + solicitar el producto **"Community Management API"** + verificar la app. **Plazo:** días a semanas, SIN SLA garantizado, puede pedir descripción de caso de uso y **puede rechazarse** → conviene **iniciar YA** para que el reloj corra en paralelo a la Fase 2. **Seguimiento semanal.** Al aprobar: Code escribe el publicador (~30 min, mismo patrón que los otros 4) + OAuth token `w_organization_social` a Railway → conectar queda en horas. Mientras tanto = semi-manual si se quisiera usar. Doc completo: `Dropbox/AUREX/REDES SOCIALES/REDES SOCIALES DE COBREX/AUTOMATIZACION_REDES_COBREX.md`.
> **🔴🎬 PENDIENTE (11-jun) — TIKTOK: grabar el VIDEO DEMO + completar requisitos + enviar a AUDITORÍA para habilitar el "Direct Post" (posteo 100% automático, sin intervención de Fernando).** Hoy TikTok quedó CONECTADO en sandbox y funciona el modo **borrador** (`video.upload`: Code sube el video → Fernando publica con 1 toque). Para el público automático hace falta la auditoría de TikTok, que exige un **video demo del flujo end-to-end con interfaz de usuario** — como nuestra integración es backend, hay que **construir una pantalla de demo real** (Login con TikTok + postear) y grabarla. Tools listos (Chrome headless + ffmpeg). App TikTok creada (`Client Key awiuia5llnl7gp7o`, sandbox `sbaw56uywxjjxzg415`), dominio cobrex.io verificado, vars en Railway (`TIKTOK_*`). Endpoints backend: `/api/tiktok/start` `/api/tiktok/callback` `/api/tiktok/verify` `/api/tiktok/test-upload` + `tiktokUploadDraft()`. Repo web cobrex.io = `fmoscon-creator/cobrex` (GitHub Pages, main, raíz).
...
> **Última actualización:** 11-jun-2026 (tarde) AR — 🤖📡 **AUTOMATIZACIÓN DE REDES COBREX: 4 de 5 canales CONECTADOS y VALIDADOS por API** (backend `aurex-backend`, deployado). ✅ **Instagram** (Graph API v21, Page Token PERMANENTE derivado en backend · `/api/ig/verify` → ok:true cobrex.io · IG Business `17841435427260864` · FB Page `1215827101606939` · FB App `1890171445007359`). ✅ **X/Twitter** (API v2, OAuth 1.0a **firmado a mano** con crypto HMAC-SHA1 · `/api/x/verify` → ok:true @cobrexio id `2060017829294776320`). ✅ **Telegram** (bot `@Aurexalertas_bot` **ya era admin** del canal `t.me/cobrexio` · `/api/tg/verify` → ok:true, puede_postear). ✅ **YouTube** (Data API v3 + **refresh token PERMANENTE** scope `youtube.upload`, app "En producción" · proyecto GCP **"Cobrex"** `tribal-mapper-499119-k5` · Brand Account de `fmoscon@gmail.com` · canal `UCXb6q8eZsHPP4nu6yYm0diA` · OAuth Client `87589926682-...` · `/api/yt/verify` → ok:true). **Cada canal = función de publicación (`igPublish`/`xPublish`/`tgPublishChannel`/`ytUpload`) + endpoint verify (gated `?k=FB_PAGE_ID`).** **Vars en Railway (solo NOMBRES; valores SOLO en Railway):** `FB_APP_ID·FB_APP_SECRET·IG_ACCESS_TOKEN·FB_PAGE_ID·IG_BUSINESS_ID` · `X_API_KEY·X_API_SECRET·X_ACCESS_TOKEN·X_ACCESS_TOKEN_SECRET` · `TELEGRAM_BOT_TOKEN` · `YT_CLIENT_ID·YT_CLIENT_SECRET·YT_REFRESH_TOKEN`. **🔴 Falta LINKEDIN** (pendiente de seguimiento arriba — aprobación Community Management API, semanas) + **Fase 2 = CONTENIDO** por canal al banco de imágenes (NADA se publica sin OK formal de Fernando). **Cosmético:** renombrar bot Telegram `Aurex→Cobrex` (se ve en la app, no en el canal · próximo build). **Doc completo:** `Dropbox/AUREX/REDES SOCIALES/REDES SOCIALES DE COBREX/AUTOMATIZACION_REDES_COBREX.md` + `_11JUN_v1.docx`. + ↓
> **Última actualización:** 11-jun-2026 — 🤖🎉 **ANDROID 1.0.39 PUBLICADO EN PRODUCCIÓN — LIVE en Google Play (177 países).** Verificado: versión **ACTIVA** en Producción = **39 (1.0.39)** (reemplazó la 37); mail de Google "Se publicó la actualización de la app". **Cierra el Build 39** = port completo de iOS 44+45 (footer color, paywall trial/USD/Gratis, 500+/Metales, idioma del dispositivo, splash sin spinner, catálogo 503 SHOP/ZM/TTWO) + Play Console: **precios 4 planes** (base + Argentina, revisados país por país), **trial 7d**, **metadata 8 idiomas** (descripción "500+" con desglose real 359 stocks/37 ETF/17 fut/6 metales **SIN precios** — guardada en `Downloads/DESCRIPCION_COMPLETA_PLAYSTORE_8IDIOMAS.txt`; novedades; beneficios de suscripción), **categoría Finanzas**, declaración AdId (Sí, Analytics). **🏆 LAS 2 PLATAFORMAS AL DÍA: iOS 1.3 (45) aprobada/publicada + Android 1.0.39 publicado.** Pendiente menor: edge-to-edge Android 15 (aviso no-bloqueante de RN core, diferido [[project_pendiente_edge_to_edge_android]]). **✅ VERIFICADO post-publicación (11-jun):** ficha pública de Play Store ya muestra **categoría Finanzas + descripciones actualizadas**; **popup de reseña CONFIRMADO funcionando en iOS (1.3) Y Android (1.0.39)** — apareció al abrir tras pasar el gate de 2 días de uso (no estaba roto, era el gate). + ↓
> **Última actualización:** 10-jun-2026 (cierre noche) AR — 🤖💰 **ANDROID BUILD 39 (1.0.39): TODA LA CONFIG DE PLAY CONSOLE TERMINADA + precios revisados país por país. LISTO PARA PROMOVER A PRODUCCIÓN.** **💵 PRECIOS de los 4 planes en Play Console (Base/EEUU · Argentina):** PRO Mensual **$4.99 · $2.99** · PRO Anual **$47.99 · $28.99** · ELITE Mensual **$9.99 · $4.99** · ELITE Anual **$95.99 · $47.99**. Google convierte la base a cada país (moneda local + IVA incluido en los que tienen). **Code revisó los 175 países de cada plan uno por uno** y corrigió/guió: ELITE Anual tenía **11 países en $94.99** (Bermudas, Camboya, Ecuador, El Salvador, Islas Caimán/Turcas/Vírgenes, Kuwait, Micronesia, Panamá, Guinea) → corregidos a $95.99; el resto quedó limpio. **Argelia** revertida a la base en los 4 planes (Escritorio le había puesto por error el descuento que era de Argentina — confundió Argelia/DZD con Argentina/ARS). **Bases de los anuales y ELITE Mensual estaban viejas** ($89.99/$19.99) → corregidas a $47.99/$9.99 con "seleccionar todo". **🎁 Free trial 7 días** (oferta `free-trial-7days`, criterio "adquisición clientes nuevos") activa en los 2 MENSUALES (PRO + ELITE); RC lo lee en runtime (no en su dashboard). **📲 METADATA/FICHA (8 idiomas, guardada SIN publicar):** descripción **350+→500+** · **Novedades 1.0.39** · **Beneficios de suscripción** (Nombre + beneficios por producto, derivados de los planes iOS Build 45; los **ANUALES** llevan un beneficio extra **"Ahorrás 20% vs el mensual"** — como Play permite máx 4, el anual = descuento + 3 features) · **Categoría → Finanzas** (era "Herramientas"). **⚠️ Escritorio descartado para precios** (cometió errores Argelia/Argentina + se quedó sin usos) → Code revisó y guió todo. **PENDIENTE:** Fernando promueve **prueba interna → producción**. Docs: `briefs/build39_android/` (ESCRITORIO_PLAYCONSOLE_39 + BENEFICIOS_SUSCRIPCIONES_8IDIOMAS). + ↓
...
## 📁 BANCO DE IMÁGENES — organización, uso, búsqueda y guardado (12-jun-2026)

**Ubicación:** `Dropbox/AUREX/BANCO IMAGENES REDES SOCIALES COBREX/`. **Idioma del contenido = INGLÉS** (consistente con la bio/marca). **Nada se postea sin OK formal de Fernando.**

**3 mundos (separados a propósito — materia prima ≠ molde ≠ producto final):**
- `00_FUENTES/` — materia prima REUTILIZABLE, NO se postea sola: `logos/`, `iconos-redes/` (7 redes + web, color), `badges-tienda/` (Apple + Google Play color), `fondos/`, `mascota-buho/`, `capturas-app/`, `capturas-web/`, `banners-perfil-canal/`.
- `01_TEMPLATES/` — moldes por FORMATO/ratio: `1x1_1080x1080`, `4x5_1080x1350`, `9x16_1080x1920`, `16x9_1920x1080`, `thumb_1280x720`, `_componentes/` (footer dorado, etc.).
- `02_CONTENIDO/` — piezas FINALES, **UNA carpeta por pieza/campaña** (`001-lanzamiento`, …). Cada pieza:
  - `meta.json` — el cerebro: `estado` (borrador|en-progreso|aprobado|publicado) + caption + hashtags + `canales{}` (archivo+estado por red) + `pendientes`.
  - `_master/` — diseño EDITABLE (HTML) + render aprobado (re-editar sin rehacer).
  - 7 sub-carpetas por red: `instagram/ tiktok/ x/ youtube/ linkedin/ telegram/ web/`. El nombre del archivo dice su tamaño (`feed-1080x1350.png`, `video-1080x1920.mp4`). **png y video van PLANOS por red (NO se separan en subcarpetas).**

**Formatos por red (referencia):** IG feed 1x1/4x5 + story 9x16 · TikTok 9x16 · X 16x9/1x1 · YouTube 16x9 + Short 9x16 + thumb 1280x720 · LinkedIn 1.91:1/1x1 · Telegram libre · Web banners + OG 1200x630.

**BÚSQUEDA:** por pieza (todo junto) + por red dentro de la pieza. **GUARDADO:** materia prima→`00_FUENTES/` · molde→`01_TEMPLATES/<formato>/` · pieza final→`02_CONTENIDO/NNN-<campaña>/<red>/`. **AUTOMATIZACIÓN:** el cron recorre `02_CONTENIDO/`, lee `meta.json` (¿`estado=aprobado`? ¿qué redes? ¿caption?) y postea el archivo de cada red. Pieza inicial: `001-lanzamiento` (imagen v5 aprobada en `instagram/feed-1080x1350.png`; pendientes: cuadrada 1x1, video 9x16, captions por canal). Estructura creada 12-jun (migró el banco viejo sin pérdida).
