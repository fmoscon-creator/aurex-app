...
- 🔴🔴 **TODA la metadata de la ficha App Store quedó en AUREX, no Cobrex** (confirmado IMG_2699, buscando "Cobrex" aparece): **Nombre "AUREX AI"** + **capturas VIEJAS de AUREX** (dicen "AUREX Alerts/Markets/PULSE" + "Alertas por WhatsApp" feature inexistente) + subtítulo "Real-time market data tracker" + **categoría "Utilidades"** (debería Finanzas) + dev "Fernando gabriel moscon". Solo el ícono coincide. El **binario SÍ es Cobrex** (Build 35, verificado adentro) — se rebrandeó el binario pero NO la metadata de ASC. **ACCIÓN:** verificar en ASC → *Información de la app → Nombre* + versión 1.0 → *Capturas*. Si dicen AUREX → cambiar TODO a Cobrex (nombre + capturas Cobrex + categoría Finanzas) y reenviar (revisión ~1 día). Si dicen Cobrex → era cache, se actualiza ≤24h. **🚫 NO promocionar hasta que la ficha pública diga Cobrex.**
