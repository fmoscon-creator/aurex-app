...
- ✅ **RESUELTO (era cache):** el **Nombre real en App Store Connect = "Cobrex"** (verificado por Escritorio, + subtítulo "AI market analytics", keywords OK). El "AUREX AI" que se veía en la tienda pública (IMG_2699) era **CACHE** de la app recién publicada → se corrige solo al propagar (≤24h). Lo mismo las capturas viejas de AUREX que se veían = cache. **Capturas en Connect = Cobrex CONFIRMADO** ✅ (Escritorio). Entonces el "AUREX AI" + capturas viejas de la tienda eran 100% CACHE → metadata RESUELTA. Ícono CONFIRMADO por Code (`icon-1024.png` Build 35) = **solo símbolo dorado, SIN texto** = el mismo emblema de Cobrex → OK, NO se toca. **🚫 No promocionar hasta que la ficha pública muestre Cobrex (esperar propagación ≤24h + reconfirmar).**
- 🔴 **Próximo BUILD iOS (Code) agrupa:** bug onboarding (logo chico/desplazado arriba) + artefacto gráfico del splash + migración links privacy/terms in-app → cobrex.io. (Ícono NO entra: es solo símbolo, ya OK.)
