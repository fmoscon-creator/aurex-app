...
-- ============================================================================
-- (1) TRIGGER PERMANENTE — cierra la causa: crea la fila usuarios en el signup,
--     sin depender de que la app se reabra. Esto es lo que "cierra" el bug.
--
--     ⚖️ DECISIÓN A TOMAR — ¿cuándo dispara?
--       OPCIÓN A (recomendada, canónica Supabase): al INSERT de auth.users
--         = en el signUp. Crea la fila al instante para TODOS, incluso si no
--         confirman el mail (la fila FREE es inofensiva). Máxima robustez.
--       OPCIÓN B: solo cuando confirman el mail (UPDATE email_confirmed_at).
--         Alineado con "registro válido = confirmado" (no crea filas para los
--         que nunca confirman). Coherente con dejar a los 12 sin confirmar.
--
--     Abajo está la OPCIÓN A activa. Para usar la B, comentá la A y descomentá la B.
-- ============================================================================
...
-- OPCIÓN A — dispara en el signup (auth.users INSERT):
drop trigger if exists on_auth_user_created on auth.users;
create trigger on_auth_user_created
  after insert on auth.users
  for each row execute function public.handle_new_user();

-- OPCIÓN B — dispara al confirmar el mail (descomentar si se prefiere esta):
