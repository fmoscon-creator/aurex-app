# PLAN ARCHIVO-POR-ARCHIVO — IAP TIER 1 (Build 34 Android)
> Fecha: 16-may-2026 02:30 AR
> Estado: **DOCUMENTADO, SIN IMPLEMENTAR** — esperando OK Fernando + Escritorio
> Autor: Code, basado en `ANALISIS_PROFUNDO_COMPRA_IAP_15MAY.md` (15-may)
...
| IAP-4 | `src/screens/PerfilScreen.js` L235-251 | `Purchases.logOut()` antes de `supabase.auth.signOut()` | Frontend RN |
...
- También revisar `deleteAccount` (L253) — agregar mismo `Purchases.logOut()` antes de `signOut()` ahí para consistencia.
...
1. **Backend primero** (IAP-5) — deploy a Railway, validar que llegan webhooks con la alerta Telegram funcionando (forzar evento de test si es posible).
2. **Frontend RN** (IAP-1 a IAP-4) — en un solo commit con bump 34.
3. **Compilar AAB Build 34**.
