...
3. **👁️ Bugs de UI/layout → validar en el SIMULADOR de Xcode ANTES de compilar el build** (Debug + Metro Fast Refresh = liviano, app ya instalada). Nunca "adivinar" leyendo código.
...
1. Code prepara los cambios de código + sube cambios al repo.
2. **Validación en simulador** de los ajustes de UI (Code) → iterar hasta OK.
3. **Compilación (Fernando):** `clean` (rm DerivedData + ios/build + reset Metro + `pod install`) → `archive + export IPA por CLI` → IPA en `~/AurexApp/backups/ipa/Build39/AurexApp.ipa`.
   - Carpeta **DerivedData por defecto**, **incremental** sobre librerías cacheadas. **NO** `-derivedDataPath` propio, **NO** build de simulador con Metro para el release.
...
|---|---|---|---|---|---|---|
| **A** | **Popup de reseña in-app** | Pedir reseña tras **3 días de USO ACTIVO** (días que abrió la app, no calendario), solo si no dejó reseña. Texto "¿Te está siendo útil Cobrex? Tu opinión nos ayuda a crecer 🙏" + "Dejar reseña" (abre link según plataforma) + "Ahora no" (re-preguntar en 7 días). Si toca "Dejar reseña" no aparece más. Máx **3 apariciones** totales. | nuevo componente RN + contador en AsyncStorage; usar `StoreReview` nativo de iOS o link directo | Código RN nuevo. Validar en simulador (forzar el contador a 3 días). | 🟡 | Code (simulador) + Fernando (TestFlight) |
| **1.h.2** | Onboarding animado | Constelación que titila + logo/COBREX más grandes (cosmético). | onboarding RN | Animación RN (Animated). Validar fluidez en simulador. | 🔴 (animación) | Code (simulador) + Fernando |
...
| **1.h.8** | Links Términos/Privacidad sin scroll | En la pantalla de planes los links requieren scrollear ("nadie adivina el scroll"). Poner línea compacta "Términos · Privacidad" **arriba**, visible sin scroll. | `SubscriptionScreen` | Reubicar/duplicar los links arriba (debajo del subtítulo). | 🟡 (UI) | Code (simulador) + Fernando |
...
> **Antes de compilar:** Code valida en simulador todos los ajustes de UI; recién con todo OK, Fernando dispara el build §11 FLUJO. **Nada se envía a Apple sin que Fernando lo apruebe y lo dispare.**
