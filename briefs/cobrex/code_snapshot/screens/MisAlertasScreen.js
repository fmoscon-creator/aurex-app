...
// Build 28 UX #8: fecha absoluta formato "19-may 14:06" (compacta, sin annio - se asume actual)
// Para que el usuario tenga referencia temporal exacta del disparo aunque pase tiempo
const MESES_ES = ['ene','feb','mar','abr','may','jun','jul','ago','sep','oct','nov','dic'];
const fmtFullDate = (ts) => {
  if (!ts) return '';
  // Build 29 B28-6 fix: Supabase devuelve disparada_at sin sufijo Z (timestamp without tz).
  // Sin Z, JS lo parsea como local time, dando hora UTC interpretada como AR (off por +3h).
  // Forzar UTC con sufijo Z deja que getHours() convierta automaticamente a TZ del device.
...
export default function MisAlertasScreen() {
  ...
  // Build 28 fix: tipo viene anidado bajo a.alertas (JOIN). Antes leia a.tipo (undefined) → !a.tipo era true → todas pasaban.
  const alertas = alertasRaw.filter(a => !a.alertas?.tipo || planAllowsAlertType(userPlan, a.alertas.tipo));
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: C.bg }} edges={['top']}>
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 14, paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: C.border, backgroundColor: C.card }}>
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
          <AurexLogo size={28} />
          <Text style={{ fontSize: 16, fontWeight: '600', color: C.gold, letterSpacing: 1 }}>{BRAND_NAME}</Text>
        ...
        <>
          ...
          <ScrollView style={{ flex: 1 }} contentContainerStyle={{ paddingHorizontal: 14, paddingBottom: 30 }}>
            {alertas.map(a => {
              ...
              // Build 28 fix Bug 4+5: los campos config llegan anidados bajo a.alertas via el JOIN del SELECT.
              // Build 27 ponia a.X directo y daba undefined → todas BAJISTA + Objetivo: ---.
              const aSrc = a.alertas || {};
              // Build 27 Bug 2.3: card con color verde/rojo + icono dinamico segun direccion de la alerta
              const isUp = aSrc.direccion === 'arriba';
              const dirColor = isUp ? C.green : C.red;
              const dirEmoji = isUp ? '📈' : '📉';
              const directionWord = isUp ? 'ALCISTA' : 'BAJISTA';
              // Build 27 Bug 2.3: linea de configuracion. Si tiene precio_base (alerta Build 27+),
              // muestra como delta desde base. Sino, muestra valor_objetivo como umbral absoluto (legacy).
              const configLine = (aSrc.tipo === 'precio_objetivo' && aSrc.precio_base != null)
                ? `${isUp ? '⬆️' : '⬇️'} ${isUp ? 'SUBA' : 'BAJA'} $${Math.abs(aSrc.valor_objetivo)} desde ${fmtP(aSrc.precio_base)}`
              ...
              return (
                ...
                >
                  ...
                  <View style={{ width: 36, height: 36, borderRadius: 18, backgroundColor: `${dirColor}26`, borderWidth: 1, borderColor: `${dirColor}55`, alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    <Text style={{ fontSize: 18 }}>{dirEmoji}</Text>
                  </View>
                  <View style={{ flex: 1 }}>
                    <Text style={{ fontSize: 14, fontWeight: '800', color: C.text, marginBottom: 2 }}>
                      {a.simbolo} — <Text style={{ color: dirColor }}>{directionWord}</Text>
                    </Text>
                    <Text style={{ fontSize: 12, color: C.text, marginBottom: 3 }}>
                      💰 Disparado: <Text style={{ fontWeight: '700' }}>{fmtP(a.precio_disparado)}</Text>
                    </Text>
                    <Text style={{ fontSize: 11, color: C.textSec, marginBottom: 4 }}>{configLine}</Text>
                    {/* Build 28 UX #8: timestamp absoluto compacto "19-may 14:06" - referencia exacta para el usuario */}
                    <Text style={{ fontSize: 10, color: C.textDim }}>{fmtFullDate(aSrc.disparada_at || a.created_at)}</Text>
                  </View>
