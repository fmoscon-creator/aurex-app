    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: C.bg }}>
        ...
        <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 12 }}>
          ...
          <TouchableOpacity
            ...
            onPress={() => {
              let msg = `⚖️ Cobrex — Comparador (${period})\n━━━━━━━━━━━━━━━━\n`;
...
export default function WatchlistScreen() {
  ...
  const doShare = (platform, sig) => {
    ...
    else if (platform === 'mail') Linking.openURL(`mailto:?subject=Cobrex IA - ${s.simbolo}&body=${enc}`);
  };
  ...
  const doShareList = async (platform) => {
    ...
    msg += `━━━━━━━━━━━━━━━━\n${ci.length} activos | Cobrex IA\nhttps://aurex.live`;
    const enc = encodeURIComponent(msg);
    setShareListModal(false);
    if (platform === 'whatsapp') Linking.openURL(`https://wa.me/?text=${enc}`);
    else if (platform === 'telegram') Linking.openURL(`https://t.me/share/url?url=https://aurex.live&text=${enc}`);
    else if (platform === 'mail') Linking.openURL(`mailto:?subject=Cobrex - Lista ${cl.name}&body=${enc}`);
  };
    ...
    <SafeAreaView style={st.container}>
      ...
      <View style={st.header}>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
          <AurexLogo size={32} />
          <Text style={st.headerTitle}>{BRAND_NAME}</Text>
