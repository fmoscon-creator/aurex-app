# PENDING REVIEW — i18n Fase 1: Código completo para verificación

---

## 1. aurex-i18n.js (ARCHIVO COMPLETO — 212 líneas)
...
## 2. pacSaveLang modificada (index.html línea 3269 — COMPLETA)
...
**Cobertura de los 8 valores del selector `<select id="select-idioma">`:**

| Valor select | map convierte a | label en labels{} |
|-------------|----------------|-------------------|
| `en` | `en` (passthrough) | English |
| `es-ar` | `es` (mapeado) | Español (Argentina) |
| `pt-br` | `pt` (mapeado) | Português (Brasil) |
| `fr` | `fr` (passthrough) | Français |
| `it` | `it` (passthrough) | Italiano |
| `zh` | `zh` (passthrough) | 中文 (Mandarin) |
| `hi` | `hi` (passthrough) | हिन्दी (Hindi) |
| `ar` | `ar` (passthrough) | عربي (Árabe) |

Los 8 valores están cubiertos. Solo `es-ar` y `pt-br` necesitan mapeo (sufijo regional → código simple). Los otros 6 pasan directo.
